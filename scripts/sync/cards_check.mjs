...
const EXPR = `(async () => {
  ...
  return JSON.stringify({
    ...
    cards: cards.length,
    rows: rows.map(r => {
      const card = r.closest(".staff-card");
      const name = card ? (card.querySelector(".sc-name")||{}).textContent : "?";
      return { coach: name, btns: [...r.querySelectorAll("a")].map(a => (a.getAttribute("title") + " -> " + a.href)) };
    })
  });
})()`;
