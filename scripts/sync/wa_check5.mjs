...
const EXPR = `(async () => {
  const src = [...document.querySelectorAll("script[src]")].map(s=>s.getAttribute("src")).filter(s=>s.indexOf("admin.js")>=0)[0] || "none";
  const appsBox = document.getElementById("apps-status");
  return JSON.stringify({src, appsHtml: appsBox ? appsBox.innerText.slice(0,400) : "no apps-status"});
})()`;
